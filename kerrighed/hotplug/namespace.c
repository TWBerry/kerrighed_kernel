 ...
 *
 * Restored for the Linux 3.10 Kerrighed port.  The namespace lifetime model
 * comes from the original Kerrighed tree; transport attachment is deliberately
 * kept out of this core restore because KRGRPC no longer uses rpc_communicator.
 */
...
int copy_krg_ns(struct task_struct *task, struct nsproxy *new)
{
	...
	struct user_namespace *user_ns = task_cred_xxx(task, user_ns);
	int retval = 0;
	...
	if (!ns && current->create_krg_ns) {
		...
		/* Only one Kerrighed cluster namespace can live at once. */
		if (!rcu_access_pointer(krg_ns)) {
			if (ns) {
...
static void delayed_free_krg_ns(struct rcu_head *rcu)
{
	...
	ns = container_of(rcu, struct krg_namespace, rcu);
...
void free_krg_ns(struct krg_namespace *ns)
{
	...
	BUG_ON(ns != rcu_access_pointer(krg_ns));
...
struct krg_namespace *find_get_krg_ns(void)
{
	...
	if (ns && !atomic_add_unless(&ns->count, 1, 0))
		ns = NULL;
	rcu_read_unlock();
